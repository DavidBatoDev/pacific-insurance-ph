...
import { Btn, Card, Field, INPUT } from "../primitives";
...
function CatalogEditor({editor,products,catalog,onClose}:{editor:Editor;products:CatalogProduct[];catalog:CatalogSnapshot;onClose:()=>void}){
  ...
  const field=(label:string,key:string,opts?:{type?:string;area?:boolean})=><Field label={label} className="mt-3">{opts?.area?<textarea className={INPUT+" min-h-20 py-2"} value={String(d[key]??"")} onChange={e=>set(key,e.target.value)}/>:<input type={opts?.type??"text"} className={INPUT} value={String(d[key]??"")} onChange={e=>set(key,e.target.value)}/>}</Field>;
  const versionSelect=<Field label="Product version" required className="mt-3"><select className={INPUT} value={String(d.versionId??"")} onChange={e=>set("versionId",e.target.value)}>{catalog.versions.map(v=><option key={v.id} value={v.id}>{v.productName} · {v.versionName}</option>)}</select></Field>;
  return <Drawer icon="fileText" title={`${editor.record?"Edit":"New"} ${editor.kind}`} onClose={onClose} footer={<><Btn onClick={onClose}>Cancel</Btn><Btn variant="primary" disabled={pending} onClick={save}>{pending?"Saving…":"Save"}</Btn></>}>
    {editor.kind==="version"&&<><Field label="Product" required><select className={INPUT} value={String(d.productId)} onChange={e=>set("productId",e.target.value)}>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>{field("Version name","versionName")}{field("Effective date","effectiveDate",{type:"date"})}{field("Expiry date","expiryDate",{type:"date"})}{field("Source document","sourceDocument")}{field("Confirmed current date","confirmed",{type:"date"})}{field("Notes","notes",{area:true})}</>}
    ...
    {editor.kind==="rate"&&<>{versionSelect}<OptionalPlan d={d} set={set} plans={plans}/><Field label="Add-on (optional)" className="mt-3"><select className={INPUT} value={String(d.addOnId)} onChange={e=>set("addOnId",e.target.value)}><option value="">None</option>{addons.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></Field><div className="grid grid-cols-2 gap-x-3">{field("Age band label","ageBand")}{field("Currency","currency")}{field("Minimum age","ageMin",{type:"number"})}{field("Maximum age","ageMax",{type:"number"})}{field("Published premium","premium",{type:"number"})}<Field label="Rate basis" className="mt-3"><select className={INPUT} value={String(d.rateBasis)} onChange={e=>set("rateBasis",e.target.value)}><option>Per Year</option><option>Per Trip</option><option>Per Coverage Unit</option></select></Field>{field("Payment mode","paymentMode")}{field("Effective date","effectiveDate",{type:"date"})}{field("Trip type","tripType")}{field("Max travel days","days",{type:"number"})}{field("Insured type","insuredType")}{field("Coverage amount","coverage",{type:"number"})}{field("Coverage unit","unit",{type:"number"})}{field("Source page","sourcePage")}</div>{field("Source document","sourceDocument")}{field("Source effective date","sourceEffective",{type:"date"})}{field("Confirmed current date","confirmed",{type:"date"})}{field("Source notes","notes",{area:true})}</>}
