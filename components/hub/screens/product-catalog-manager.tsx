  ...
  return <Card className="mt-6 overflow-hidden">
    <div className="border-b border-border-soft px-5 py-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <select className={DRAWER_INPUT+" max-w-[260px]"} value={productId} onChange={e=>{setProductId(e.target.value);setVersionId("");}}>{products.map(p=><option key={p.id} value={p.id}>{p.name}{p.quoteOnly?" · Quote-only":""}{!p.active?" · Inactive":""}</option>)}</select>
        <select className={DRAWER_INPUT+" max-w-[380px]"} value={selectedVersion?.id??""} onChange={e=>setVersionId(e.target.value)}>{versions.map(v=><option key={v.id} value={v.id}>{v.versionName}{!v.active?" · Inactive":""}</option>)}</select>
      </div>
    </div>
    {!selectedVersion?<div className="p-8 text-center text-[13px] text-subtle">No catalog version exists for this product.</div>:<>
      ...
      <CatalogSection title="Published rates" count={rates.length} onAdd={canEdit?()=>setEditor({kind:"rate",versionId:vid}):undefined}>
        <input className={DRAWER_INPUT+" mb-2 max-w-[320px]"} placeholder="Search plan, age, trip or source key…" value={rateSearch} onChange={e=>setRateSearch(e.target.value)}/>
...
function CatalogEditor({editor,products,catalog,onClose}:{editor:Editor;products:CatalogProduct[];catalog:CatalogSnapshot;onClose:()=>void}){
  ...
  const field=(label:string,key:string,opts?:{type?:string;area?:boolean})=><DrawerField label={label} className="mt-3">{opts?.area?<textarea className={DRAWER_INPUT+" min-h-20 py-2"} value={String(d[key]??"")} onChange={e=>set(key,e.target.value)}/>:<input type={opts?.type??"text"} className={DRAWER_INPUT} value={String(d[key]??"")} onChange={e=>set(key,e.target.value)}/>}</DrawerField>;
  const versionSelect=<DrawerField label="Product version" required className="mt-3"><select className={DRAWER_INPUT} value={String(d.versionId??"")} onChange={e=>set("versionId",e.target.value)}>{catalog.versions.map(v=><option key={v.id} value={v.id}>{v.productName} · {v.versionName}</option>)}</select></DrawerField>;
