...
| Migration `0036` deployment | Idempotent source migration renames the legacy proposal integration key, adds the Travel portal row and corrects two BC Flexi requirement notes | Equivalent data changes were applied and verified remotely with the service-role client on 2026-08-24. The Supabase MCP required fresh OAuth during this session, so the remote migration ledger was not advanced; replay `0036_carrier_response_reconciliation.sql` through the normal migration channel once MCP access is restored. |
